...
function checkKeycode(e) {
  ...
  if (window.event)
    ...
    {keycode = e.which;}
  if (keycode == 13){ // FIRE! or Computer - whichever is active
...
if (Secure){
  console.log("Secure is "+Secure+" and document.location is "+document.location+", so connecting to wss://" + document.domain + '/ws');
  var ws = new WebSocket('wss://' + document.domain + '/ws');
}
else {
  if (location.port.length < 1) { var port = "80"; }
  else { var port = location.port; }
  console.log("Secure is "+Secure+" and document.location is "+document.location+", so connecting to wss://" + document.domain + ':' + port + '/ws');
  var ws = new WebSocket('ws://' + document.domain + ':' + port + '/ws');
}
ws.onmessage = function (event) {
  ...
  if (messageIn.Type == "SALUTE"){
    ...
    for (y = 0; y < messageIn.FriendlyLayout.length; y++){
      for (x = 0; x < messageIn.FriendlyLayout[y].length; x++){
        if (messageIn.FriendlyLayout[y][x] == 1) {
          document.getElementById("1,"+x+","+y).classList.add('fort');
        }
      }
    }
    wigwags(messageIn.Message);
    console.log("Game started for player with ID "+messageIn.ID);
  }
  if (messageIn.Type == "ShotOut"){ // handle cell check response
  ...
  if (messageIn.Type == "SplashOver"){ // handle FIRE response
    // console.log("Status of cell messageIn. Data looks like " + messageIn.Data);
    ...
    let newClass = 'miss';
    if ((status == 1) || (status == 3)){ // if it's a hit (new or old), mark it red
      targetColor = "#f40030";
      newClass = 'hit';
    }
    if (status == 2){ // if it's a miss (new or old), mark it yellow
      targetColor = "#d8c477";
    }
  ...
  if (messageIn.Type == "FortSunk"){ // handle destroyed fort message
    targetColor = "#780015";
    var cells = document.getElementsByClassName("enemycell");
    var fort = messageIn.Fort;
    for (j = 0; j < fort.length; j++) { // for each cell in the sunk fort
      var targety = fort[j][0];
      var targetx = fort[j][1];
      for (i = 0; i < cells.length; i++) { // scan through all board cells looking for that cell
        if (targetx == cells[i].dataset.x && targety == cells[i].dataset.y) {
  ...
  if (messageIn.Type == "Incoming"){ // handle rounds in from computer
    // console.log("Status of cell messageIn. Data looks like " + messageIn.Data);
    ...
    if (status == 2){ // if it's a miss, mark it yellow
...
function clickCell(cell) { // change a cell's color when clicked
  ...
  if (cell.style.backgroundColor != "rgb(255, 255, 0)") { // if it isn't yellow, save its color, make it yellow, and set target
    deselectAll();
    ...
    ws.send('{"Type":"ShotOver","Cell":[' + cell.dataset.y + ', ' + cell.dataset.x + ']}'); // ws msg to click given cell
  } else { // if it IS already yellow, put it back to its old color and unset target
    deselectAll();
...
fireButton.onclick = function() {
  ...
  // ws.send('{"Type":"FireForEffect","Verify":"'+document.getElementById("statusVerify").value+'","Cell":['+y+', '+x+']}');
  ws.send('{"Type":"FireForEffect","Cell":['+y+', '+x+']}');
};
var computerButton = document.getElementById('computerBtn');
computerButton.onclick = function() {
  // var verify = document.getElementById("statusVerify").value;
  // ws.send('{"Type":"SplashOut","Verify":"'+document.getElementById("statusVerify").value+'"}');
...
ws.onopen = function (event) {
  ...
  ws.send('{"Type":"Recon","Difficulty":'+difficulty+'}'); // ws message back to server to get initial state of board    
}
ws.onclose = function (event) { // show message if websocket drops
